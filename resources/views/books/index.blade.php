            ...
            @foreach ($books as $book)
                <div class="w-full md:max-w-[280px]  rounded-md p-[1rem] shadow-lg">
                    ...
                    <div class="flex items-center justify-between mt-[1rem]">
                        ...
                        <a href={{ $book->status == 'finished' ? '/books/finished' : '/books/reading' }}>
                            <button
                                class="{{ $book->status == 'finished' ? 'bg-[#4ADE80]' : 'bg-[#C084FC]' }} p-[0.4rem] rounded-md text-sm capitalize hover:text-gray-500 cursor-pointer">{{ $book->status }}</button>
                        </a>
                    </div>
